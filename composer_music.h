...
* This is the header file to give old nokia phone composer music support to
* arduino song playing.
...
void getNext(char *song, char *note) {
  ...
  // skip spaces
  while (song[songIndex] == ' ') {
    songIndex++;
  }

  int i = 0;
  while (song[songIndex] != ' ' && song[songIndex]) {
    note[i++] = song[songIndex++];
  }
  note[i] = '\0';
}
